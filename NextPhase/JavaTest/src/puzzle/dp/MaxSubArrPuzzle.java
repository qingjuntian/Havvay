 ...
 * Approaches:
 *   maxSubArr  - right-to-left suffix-DP variant;
 *   maxSubArr2 - standard left-to-right Kadane scan with O(1) extra space.
 * Complexity:
 *   suffix-DP  = O(n) time, O(n) space;
 *   Kadane     = O(n) time, O(1) space.
 */
public class MaxSubArrPuzzle implements Puzzle {
    /**
     * Print the best subarray sum and its boundaries for a sample input.
     */
    @Override
    public void resolve() {
    ...
    /**
     * Right-to-left suffix DP. result[i] stores [bestSumStartingAtI, startIndex, endIndex].
     */
    ...
    /**
     * O(1)-space Kadane scan; also returns [maxSum, start, end].
     */
